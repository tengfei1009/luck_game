...
export default function Home() {
  ...
  function handleTest(ind: number) {
    ...
    // 如果传入的索引值为 1，则为请求参数添加密码字段
    if (ind === 1) {
      params['pwd'] = '123456';
    }
  
    // 根据传入的索引值 ind 执行不同的异步操作
    switch (ind) {
      case 0:
        // 调用 GetTest 函数，传入请求参数
        GetTest(params).then(res => console.log('GetTest:>> ', res));
        break;
      case 1:
        // 调用 GetTest2 函数，传入请求参数
        GetTest2(params).then((res) => console.log('GetTest:>> ', res));
        break;
      case 2:
        // 调用 GetTest3 函数，传入请求参数
        GetTest3(params).then((res) => console.log('GetTest:>> ', res));
        break;
      case 3:
        // 调用 GetTest4 函数，不传入参数
        GetTest4().then((res) => console.log('GetTest:>> ', res));
        break;
      default:
        // 如果传入的 ind 值不匹配任何 case，则抛出错误
        throw new Error('Invalid ind value');
    }
  }
  ...
  return (
    <View className="index">
      ...
      <Button
        onClick={() => Taro.navigateTo({ url: '/pagesBook/info/index' })}
      >
        info
      </Button>
